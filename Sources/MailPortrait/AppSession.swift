...
@MainActor final class AppSession: ObservableObject {
    ...
    init(arguments: [String] = CommandLine.arguments, isolatedRoot: URL? = nil) {
        ...
        if liveForeground {
            ...
            let placeholder=FileManager.default.temporaryDirectory.appendingPathComponent("mailportrait-opening-"+UUID().uuidString)
    ...
    private func openLiveLibrary()async {
        ...
        do {
            ...
            while lease == nil {
                ...
                if lease != nil {break}
                guard Date()<deadline else {throw PortraitError.message("Background sync is taking longer than expected. Close and reopen MailPortrait to retry; your library is unchanged.")}
