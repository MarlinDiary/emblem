...
@MainActor enum BackgroundService {
    ...
    static let registeredBundlePathKey="MailPortraitBackgroundServiceRegisteredBundlePath"
    ...
    static func shouldRefreshRegistration(isEnabled:Bool,registeredBuild:String?,currentBuild:String,registeredBundlePath:String?,currentBundlePath:String)->Bool {
        isEnabled && (registeredBuild != currentBuild || registeredBundlePath != currentBundlePath)
    }
    static func update(for model:AppModel)async {
        do {
            if model.mailSync.background && model.mailSync.enabled {
                let defaults=UserDefaults.standard
                if shouldRefreshRegistration(isEnabled:service.status == .enabled,registeredBuild:defaults.string(forKey:registeredBuildKey),currentBuild:currentBuild,registeredBundlePath:defaults.string(forKey:registeredBundlePathKey),currentBundlePath:currentBundlePath) {
