...
struct MailPortraitApp: App {
    ...
    var body: some Scene {
        ...
        .commands {
            CommandGroup(replacing: .appTermination) { Button("Quit MailPortrait") {NSApp.terminate(nil)}.keyboardShortcut("q") }
        ...
        Window("Photo Sources · MailPortrait", id: "sources") { SourcePreviewView(model: session.model).frame(minWidth: 820, minHeight: 600).preferredColorScheme(session.appearance) }.defaultSize(width: 1000, height: 760)
        Window("MailPortrait Help", id: "help") { HelpView().frame(minWidth: 480, minHeight: 500) }.defaultSize(width: 540, height: 620)
    }
}

struct HelpCommand: View {
    @Environment(\.openWindow) private var openWindow
    var body: some View { Button("MailPortrait Help") { openWindow(id: "help") } }
}
