...
@MainActor final class AppModel: ObservableObject {
    ...
    init(demo: Bool, rootOverride: URL? = nil, resolverFactory: @escaping () -> AvatarResolver = { AvatarResolver() }, mailScanner: (any MailScannerPort)? = nil, contactScanner: (any ContactsScannerPort)? = nil,backgroundWorkAllowed:Bool=true) {
        ...
        self.demo = demo
        root = rootOverride ?? FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0].appendingPathComponent(demo ? "MailPortrait-Demo" : "MailPortrait", isDirectory: true)
        if demo {
    ...
    func ensureContactsAccess() async throws {
        if !allowsPermissionPrompts && !demo && CNContactStore.authorizationStatus(for:.contacts) != .authorized {throw PortraitError.message("Open MailPortrait to allow Contacts access.")}
