            ...
            do {
                ...
                if source == .contacts {
                    if !self.demo && self.usesLiveContactScan {
                        if !self.allowsPermissionPrompts && CNContactStore.authorizationStatus(for:.contacts) != .authorized {throw PortraitError.message("Open Emblem to allow Contacts access.") }
