...
@MainActor final class AppleContacts: ContactStorePort {
    ...
    func requestAccess() async throws {
        ...
        guard status == .notDetermined else {
            throw PortraitError.message("Allow full Contacts access for Emblem in System Settings → Privacy & Security → Contacts.")
        }
        do {
            ...
            guard granted else { throw PortraitError.message("Allow Emblem in System Settings → Privacy & Security → Contacts, then reconnect.") }
        } catch {
            throw PortraitError.message("Contacts access is not enabled. Check Emblem in System Settings → Privacy & Security → Contacts. System details: \(error.localizedDescription)")
        }
    ...
    func execute(_ request: CNSaveRequest) throws { try requireFullAccess(); request.transactionAuthor = "com.protoyard.emblem"; try store.execute(request) }
    ...
    func update(id:String,image:Data?,emails:[String]) throws -> ContactSnapshot {
        ...
        // aliases Emblem adds. Replacing the array indiscriminately would
