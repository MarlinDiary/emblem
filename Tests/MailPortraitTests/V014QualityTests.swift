...
final class V014QualityTests:XCTestCase {
 @MainActor func testRealClaudeMigrationUsesBundledFirstPartyVectorWithoutNetwork()async throws {
  ...
  if let folder=ProcessInfo.processInfo.environment["MAILPORTRAIT_V014_ASSETS"] {
 ...
 @MainActor func testRealStreamlinedInterfaceSnapshots()async throws {
  guard let folder=ProcessInfo.processInfo.environment["MAILPORTRAIT_V014_ASSETS"],let snapshots=ProcessInfo.processInfo.environment["MAILPORTRAIT_SNAPSHOT_DIR"] else{throw XCTSkip("real visual corpus opt-in")}
  let root=FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString);defer{try? FileManager.default.removeItem(at:root)}
