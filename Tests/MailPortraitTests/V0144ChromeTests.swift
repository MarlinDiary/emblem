...
final class V0144ChromeTests:XCTestCase {
    private var sources:URL {URL(fileURLWithPath:#filePath).deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent().appendingPathComponent("Sources/MailPortrait")}
    func testSidebarOverlayCannotDefaultToHorizontalDivider()throws {
        let source=try String(contentsOf:sources.appendingPathComponent("MailPortraitApp.swift"))
    ...
    @MainActor func testBoundaryPixelsStayOnRightEdgeInLightAndDark()async throws {
        for dark in [false,true] {
            ...
            if let dir=ProcessInfo.processInfo.environment["MAILPORTRAIT_SNAPSHOT_DIR"] {
