...
final class V015SurfaceTests:XCTestCase {
    func testToolbarDoesNotContainFilterOrMislabelCurrentPhotoAsSource()throws {
        let source=URL(fileURLWithPath:#filePath).deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent().appendingPathComponent("Sources/MailPortrait")
        XCTAssertFalse(try String(contentsOf:source.appendingPathComponent("MailPortraitApp.swift")).contains("ToolbarItem(id:\"sender-filter\""))
