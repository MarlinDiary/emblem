...
final class SnapshotTests: XCTestCase {
    ...
    @MainActor func testOffscreenLayoutExports() async throws {
        guard let path = ProcessInfo.processInfo.environment["EMBLEM_SNAPSHOT_DIR"] else { throw XCTSkip("Set EMBLEM_SNAPSHOT_DIR for native rendering checks") }
        let output=URL(fileURLWithPath:path, isDirectory:true)
        try FileManager.default.createDirectory(at:output,withIntermediateDirectories:true)
        let root=FileManager.default.temporaryDirectory.appendingPathComponent("emblem-snapshots-"+UUID().uuidString)
    ...
    @MainActor func testRealLibraryWorkflowExports() async throws {
        guard let path = ProcessInfo.processInfo.environment["EMBLEM_SNAPSHOT_DIR"],
              let state = ProcessInfo.processInfo.environment["EMBLEM_REAL_STATE"] else { throw XCTSkip("Real-state rendering is opt-in and read-only") }
        let output = URL(fileURLWithPath: path, isDirectory: true)
        try FileManager.default.createDirectory(at: output, withIntermediateDirectories: true)
    ...
    @MainActor func testV011ChoiceSurfaceWithRealAssets() async throws {
        guard let path=ProcessInfo.processInfo.environment["EMBLEM_SNAPSHOT_DIR"],
              let state=ProcessInfo.processInfo.environment["EMBLEM_REAL_STATE"],
              let original=ProcessInfo.processInfo.environment["EMBLEM_LINKEDIN_ORIGINAL"] else { throw XCTSkip("Real artwork rendering is opt-in") }
        let output=URL(fileURLWithPath:path,isDirectory:true)
        try FileManager.default.createDirectory(at:output,withIntermediateDirectories:true)
