...
final class V012ExperienceTests:XCTestCase {
    ...
    @MainActor func testMonogramUsesBackingPixelThumbnail() throws {
        ...
        if let folder=ProcessInfo.processInfo.environment["EMBLEM_V012_ASSETS"] {
    ...
    @MainActor func testRealCacheMigrationAndFreshOriginalCanvas() async throws {
        guard let folder=ProcessInfo.processInfo.environment["EMBLEM_V012_ASSETS"],let state=ProcessInfo.processInfo.environment["EMBLEM_REAL_STATE"] else { throw XCTSkip("read-only corpus opt-in") }
